...
不创建 `v2` 长期目录名。清理后的仓库没有旧项目管理 action 或组件；新增目录直接作为唯一项目管理实现。
...
旧项目管理页面、action、组件、validation、permissions、通知事件、cron、测试、专用 schema 和 seed 已在本计划实施前删除。后续实现必须遵守：

- 不恢复旧 ProjectStage、立项、DDL、WeeklyReport 或旧 Task submission 接口。
- 不增加 legacy adapter、旧数据导入脚本或双写。
- 不把采购/反馈 adapter 扩展成项目管理分支；项目管理使用独立 channel adapter。
- `/progress` 已替换为新入口；旧地址处理继续由路由测试锁定，不恢复旧 Project/Stage/Task 语义。
