...
2. 初始尺度同时考虑当前逻辑范围跨度和可用像素宽度，而不是使用已加载 leaf blocks 或只按天数判断。
...
边界行为同时冻结为：上一/下一先按当前可视时长的 80% 计算目标中心；目标仍在当前逻辑窗口时只
滚动。内容驱动页面若目标越过 3 年逻辑窗口但完整内容范围在该方向仍有数据，则按 D5 的日历算法
围绕目标重建窗口，使用 `replaceState` 更新 `center`，生成覆盖新逻辑窗口的 `rowPageKey` 并按 D19
清空旧块后预取；到完整内容边界后禁用对应方向。`EXPLICIT_FILTER` 不重建范围，到硬边界后禁用。
“今天”只有在完整内容加缓冲范围或显式硬范围内才启用；在当前逻辑窗口外但完整内容范围内时按同
样规则重建，在完整范围外时禁用并提示“今天不在当前时间范围内”。这些显示导航不创建业务记录。
...
1. 新增内容边界、上海日历月缓冲、自适应尺度、视口中心保持和导航移动的纯函数。
...
1. 实现目标日期轴、尺度选择、“今天”、上一/下一、D16 行标题宽度和底部同步滚动条。
...
1. D13=A/C 时四种尺度、D13=B 时六种尺度在已决定的适用页面可用；切换后不丢选择、不重置行
   分页、不触发无关业务查询。
2. 首次尺度同时考虑范围和容器宽度；用户手动选择后，窗口 Resize 不覆盖其选择。
...
9. 键盘可操作尺度、今天、上一/下一、滚动区域和时间对象；焦点可见，ARIA 名称明确。
