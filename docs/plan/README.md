...
- 代码基线：`main` 分支提交 `44b51d673fe48914a7abefb4e3645d3496e48c2f`。
- 现有技术栈：Next.js 16、React 19、TypeScript、Prisma 7、PostgreSQL、Auth.js、飞书、通知 outbox、Playwright。
- 仓库现有业务：立项审批、ProjectStage 阶段验收、Task 交付验收、DDL 变更申请、周报、风险、通知提醒、角色管理。
...
“无需向后兼容”指不保留旧功能和旧数据结构的运行时兼容。迁移时旧 Project 和旧 Task 各自转换为新 Task；除构建新 Task 所需的名称、描述、状态、负责人、参与人、范围和截止信息外，其他旧项目管理数据直接删除。
...
| [04-数据库设计与迁移映射.md](./04-数据库设计与迁移映射.md) | 表、约束、索引、旧数据映射 | 后端、DBA |
...
| [09-数据迁移发布与回滚.md](./09-数据迁移发布与回滚.md) | 扩表、回填、切换、回滚 | 技术负责人、DBA、运维 |
...
- 旧流程切换后直接删除旧 UI、action、权限分支、提醒和测试，不保留双写或兼容适配层。
- 旧 Project 与旧 Task 均一对一迁成新 Task；旧 Project 名称转换成 Tag。
- 旧 Stage、提交审批、DDL、WeeklyReport、风险、评论、关注和提醒记录不迁移。
- 自动化测试禁止发送真实飞书。
