...
| 字段 | 说明 |
|---|---|
| `legacySourceType/legacySourceId` | 只用于标明由旧 PROJECT/TASK 迁移，组合唯一 |
| `title/description` | Task 元数据 |
| `scopeTeam/scopeTechGroups` | Task 自身组织范围，不从 Tag 继承 |
...
`TaskMember` 保存 Person 与 Task 的角色：OWNER/LEAD/MEMBER/REVIEWER/VIEWER。至少一个 OWNER。
...
| 旧对象 | 新对象 | 规则 |
|---|---|---|
| `User` | Account + FEISHU Identity + Person | 每个唯一 openId 建一组映射；unionId 存 provider subject |
| `Project` | Task + Tag | 每条旧 Project 生成一条新 Task；Project.name 同时生成 Tag |
| `Task` | Task + Tag | 每条旧 Task 生成一条新 Task；若原属 Project，则关联对应 Project 名称 Tag |
| `ProjectOwner/Participant` | TaskMember | 仅作为旧 Project 转成的新 Task 成员 |
| `TaskAssignee` | TaskMember | 作为旧 Task 转成的新 Task Owner |
| 顶层 goal/description/metrics/dueAt | 初始 Milestone | 每个迁移 Task 只生成一个 Milestone 和一个 Termination |
| 旧 Project/Task 结束状态 | Task + Termination | 按直接状态映射 |
| 其他所有旧项目管理表 | 删除 | Stage、submission、approval、DDL、weekly、risk、comment、follow、reminder 均不迁移 |

旧 Task 的 `TODO` 迁移为 `DRAFT`；`IN_PROGRESS/PENDING_ACCEPTANCE` 迁移为 `ACTIVE`；`COMPLETED` 迁移为 `COMPLETED`；`ARCHIVED` 迁移为 `ARCHIVED`；`PROJECT_CANCELED` 迁移为 `CANCELLED`。旧 Project 按其顶层状态直接映射，详见迁移 runbook。

## 12. 删除旧 schema

完成迁移对账并获得业务签字后，新的收缩 migration 删除旧项目管理专用：

- Project、ProjectStage 与其 owner/participant/risk/DDL 表。
- 立项审批字段。
- Task 旧状态字段和旧 assignee/techGroup 冗余字段。
- TaskSubmission、ApprovalRecord 中只服务旧进度的关系。
- Task 创建/删除/DDL Request。
- Submission/Approval、WeeklyReport、旧 Risk、Comment、旧 Follow、旧 Reminder 类型。
- 对应 enum、索引和外键。

若某表也服务采购或反馈，不删除表，只删除项目管理专用字段/记录类型。执行前必须由 `rg` 代码引用清单和数据库外键清单双重确认。
