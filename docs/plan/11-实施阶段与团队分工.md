...
> 状态说明：P0–P8 是最初的领域建设阶段；S0–S10 是 2026-07-30 起 P4–P8 的实际执行/关单顺序。S1 已完成 P5-R01～R05 的服务端重新关单，但 P5-12/P5-13 仍在 S9；`345b5b0` 只构成首批 P4/P6 卡片/列表 UI 和项目管理通知 adapter 基线，不能据此把整个 P5、P4/P6 或 TimeCanvas 标记为完成。
...
| P5 Segment/Conflict 主体 | 已实现（S1 服务端重新关单）；计划（S9 运维/性能） | `e0317cc` + `2499952` handoff 是历史基线；P5-R01～R05 由 `87635a5b`、`bb72c469`、`414d14f4`、`8d0e494` 关单；P5-12/P5-13 留在 S9 |
...
| 阶段 | 目标 | 主要关单门禁 | 映射原阶段 |
|---|---|---|---|
| S0 | 基线、ADR、计划和追踪矩阵校正（已完成） | `f96ddd37` 冻结执行基线、handoff/345b5b0 时序和单一状态口径 | P4–P8 协调 |
| S1 | P5 安全、并发与不变量重新关单（已完成） | P5-R01～R05 对应提交与回归齐全；person 级 transaction advisory lock、首次 fingerprint、隐私/并发与全量 E2E 已关单 | P5（不含 P5-12/P5-13） |
...
当前状态：S0 已由 `f96ddd37` 冻结执行基线，S1 已由 `87635a5b`、`bb72c469`、`414d14f4`、`8d0e494` 完成；S2–S10 仍为计划。S3–S8 的前端工作没有因 S1 关单而提前完成。
...
历史与当前状态：`2499952` 中的 handoff 是 `e0317cc` 在 2026-07-29 01:00 CST 的历史快照；`345b5b0` 后续加入 adapter 和首批 UI，`f96ddd37` 冻结当前执行基线。S1 随后再次关闭了以下 P5-R01～R05 遗留：
...
- Conflict 扫描按 person 使用 transaction advisory lock，数据库级互斥和首次 fingerprint 竞争已在 S1 完成验证，未留到 S9。
...
| 工作包 | 状态 | 关单提交 | 验收证据 |
|---|---|---|---|
| P5-R01 | 已实现 | `87635a5b` | Conflict capability、preview/处理同权、隐藏 Segment 与处理文本脱敏、跨 Task scope/owner 拒绝路径；定向 Conflict spec，并由 R05 全量回归复验。 |
| P5-R02 / P5-R03 | 已实现（同一提交） | `bb72c469` | guarded transition、固定行锁顺序、person 级 transaction advisory lock、首次 fingerprint、reopen、scanner/人工处理竞争与 exactly-once；归档记录 `npm run check`、四个 spec 双视口 `104/104`、关键 13 标题三轮 `39/39`。 |
| P5-R04 | 已实现 | `414d14f4` | merge 31 天与缺失 Allocation 全量证据回归在 `bb72c469` 已落地；`414d14f4` 的三份受影响 Playwright spec 覆盖人工/系统操作者、ignore 不误报解决、切换后 Current Plan 上下文和通知 adapter，归档状态为通过（未保留可靠引用的用例数）。 |
| P5-R05 | 已实现 | `8d0e494` | 最终关单覆盖真实 100 条末项 stale 全回滚、来源、stale apply、并发与外联安全；`npm run check` 通过，三个受影响 spec 双视口 `62/62`，`npm run build` 通过，全量 E2E `326 passed`、30 条既有条件性 skip、`0 failed`，`strace` `16/16` 且无 DNS、非回环 INET、Feishu/Lark/Prisma checkpoint 外联。 |

P5 总体测试范围（S1 已完成项与 S9 待完成项合并列示）：
...
- S1 门禁已满足：P5-R01～R05 对应提交、拒绝路径、事务/并发/通知回归和最终全量验证齐全；所有 Segment 操作不改变 Task/Milestone。
- P5 总体门禁尚未满足：P5-12/P5-13 的 cron 跨实例运维、checkpoint/增量与每日完整扫描、100k Segment 性能数据和索引报告仍须在 S9 达标。
