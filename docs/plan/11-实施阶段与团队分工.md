...
| 数据迁移 | TL | BE-A/DBA | BO/QA/BE-B | 全员 |
| 删除旧代码/schema | TL | BE-A/BE-B | DBA/SEC/QA | BO |
...
gantt
  ...
  section 发布
  P7 迁移与删除         :p7, after p3, 8d
  P8 回归UAT上线        :p8, after p6, 8d
```
...
- 冻结旧 Project/Task 状态与数据量。
- 明确删除清单和采购不受影响清单。
- 建立 UAT 场景与性能数据规模。
- 输出 ADR、状态转换表、权限矩阵和迁移字段表。
...
## 11. P7：迁移、删除旧功能与文档（6-8 个工作日）

负责人：TL(A)、BE-A/BE-B/DBA(R)、QA/SEC(C)、BO(I)。

工作：

- 完成 Project -> Task、Task -> Task。
- 旧 Project 名称 -> Tag。
- 完成两次生产快照演练。
- 建立数据和删除对账。
- 删除旧页面、actions、permissions、notifications、cron、tests。
- contract migration 删除旧表。
- 更新 README/TECH/TESTING/NOTIFICATIONS。

退出门禁：

- `new Task = old Project + old Task`。
- 删除清单完全匹配。
- 采购数据 hash 不变。
- 整库恢复演练通过。
...
- 每个 WBS 工作包一个或数个小 PR。
- schema expand、领域服务、UI、migration、删除旧代码分开。
- 不把“新增新功能”和“删除旧功能”混在一个难审查 PR。
- 每个 PR 描述状态、权限、审计、通知、测试和迁移影响。
- 每个阶段至少两轮独立审查；高风险 migration/权限由 SEC/DBA 参与。
- contract migration 在所有新功能合并、演练通过后最后提交。
