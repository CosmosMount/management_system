...
- 所有已登录并成功解析到统一 `Account/Person` 的账号都可调用 `createTaskDraft`，车组和技术组仍必须使用固定合法值；创建者由服务端自动归一化为 OWNER。创建者 Person 即使已停用也保留该自动 Owner，其他新增成员必须是活跃 Person。
...
- 所有已登录并成功解析到统一 `Account/Person` 的账号可读取全部未逻辑删除 Task、计划版本、成员、Milestone Review、Revision、Task 审计和全员完整 Work Segment/变更历史；账号模型不再提供项目访问启用/禁用状态。
- Person 的 `INACTIVE` 是通讯录业务状态，不阻止登录、全局读取或创建 Task；停用 Person 不可作为其他 Task 的新增成员，也不可创建新 Segment，其历史成员和历史 Segment 继续展示。逻辑删除对象仍不能通过搜索或显式 ID 枚举。
...
- 所有已登录统一账号可查看所有未删除 Segment 的完整内容和变更历史。
