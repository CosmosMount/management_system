...
- `createTaskDraft` 的 active members 必须恰好一个 OWNER；0 OWNER、2 个及以上 OWNER、同一 Person 重复相同 role 均拒绝。同一 Person 可兼任多个不同 role，权限取并集。
...
- `replaceTaskDraftMembers`：只整包替换成员与角色，保持恰好一个 active OWNER，拒绝同一 Person 重复相同 role；同一 Person 可兼任多个不同 role，不接受 metadata 或计划字段。
...
- `replaceTaskMembers`：整包差异更新 active members；移除使用 `removedAt` 保留历史，结果恰好一个 active OWNER，拒绝同一 Person 重复相同 role，并允许同一 Person 兼任多个不同 role。
...
系统默认固定为 `REVIEW_REQUIRED`。`DIRECT_BY_OWNER` 只能作为 Task 策略显式配置；改变全局默认必须走 ADR 和变更控制。
