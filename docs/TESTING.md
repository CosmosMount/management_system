...
5. Planned 完整确认后只显示 Actual；前缀部分确认不显示也不要求原因输入，必须填写实际投入内容、预期产出、实际产出，固定开始点并只生成一条尾段。缺字段或伪造中间起点必须在服务端零写入拒绝。确认、取消、Actual 软删除仍需验证数据库、来源、中文安全 change DTO、audit、站内通知与 outbox；历史分页不得下发 raw `before/after`、账号 ID 或无权读取的 Task 名称，测试环境必须禁用真实飞书投递。普通 DTO、表单和最终数据库均不得包含 `completionPercent`；迁移前非空值只能从 append-only 领域审计查询。
...
6. Desktop `1440x1000` 与 Pixel 5 打开 `/progress/projects/[id]`：概览只显示头像、名称、完整内容、状态、负责人、参与人和 Task 完成进度，权限操作位于右上；不得显示立项历史或 raw 审计卡片，但 `#establishment` 仍定位审批区。第二段左侧将 Project 自身风险与当前所属 Task 风险分组，并只显示 Project 评论；中间按“草稿 → 进行中 → 所有终态”展示全部 Task、只包含本 Project Task 的 Current Plan 时间线及风险输入，同时展示 Project/Task 全部有效成员在其他 Task 和独立投入中的完整时间；右侧显示服务端筛选和稳定分页的中文近期动态。已完成节点使用绿色勾选，未完成节点保留普通圆点；定位按钮仍选择当前进行中的非 Revision 节点，没有 Active 节点时回退到 Start。两种视口都要验证无横向溢出、全量 Task/人员投入和投入悬浮 Task 信息。
