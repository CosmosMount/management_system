...
6. 在“每日卡片”中配置多个发送时间，确认可增删、按时间排序持久化，重复时间、少于 5 分钟间隔和超过 8 个时间均被阻止。
7. 手动触发提醒扫描，期望生成通知 outbox 或发送记录。
8. 对失败 outbox 执行重试，期望状态变化且不重复发送已成功收件人。
9. 触发飞书用户同步，期望同步结果 toast 显示新增/更新数量。
...
5. 重跑 drain 不重复发送相同 `eventKey`，多收件人通知只重试失败的 `NotificationOutboxRecipient`。
6. 到期任务、逾期任务、缺失周报、阶段临期提醒应按管理员提醒规则生成通知。
7. 每日卡片同日不同计划时刻应生成不同 outbox；同一场次重复扫描不重复入队，积压多个场次只执行最近一场。
8. 同时启动多个 cron 时，应确认不会重复执行同一次扫描；如发现重复，记录为并发风险。
