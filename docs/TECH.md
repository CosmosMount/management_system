...
时间画布查询默认排除 `PLANNED + CONFIRMED/CANCELLED`，但不删除事实记录、来源和历史。PERSONAL scope 的 Task universe 来自有效 TaskMember；“我的时间”通过 `getMyTimelinePageData` 只接受 `taskCursor/showAll`，服务端按稳定游标派生当前 25 条参与 Task，再把同页 Current Plan 与本人可见投入装配到同一画布，客户端没有注入 Task/Person ID 的入口。内容驱动画布在授权过滤后聚合 `min(startAt)/max(endAt)` 与 Current Plan 时间，向外对齐两个上海日历月，最多显示三个上海日历年；兼容数据的 `plannedStartAt=null` 仍保留原值，只用 Task `createdAt` 作为只读 Start marker 和范围边界。查询按 180 天块读取，单块超过 5,000 对象时按上海自然日自动二分，并执行 20,000 对象/16 leaf block 双预算。常规 DTO 返回稳定 `rowPageKey`；它用于识别结构版本，不代替每次查询的鉴权。Task 详情以有效 TaskMember 为人员范围，Project 详情以 ProjectMember 与当前 Task 页 TaskMember 并集为范围；旧 `date/mode/timelineDate/timelineFocus` 仅转换为新的 `center/scale/focus` 显示状态。人员计划的显式 `from/to` 仍是硬业务范围。

TimeCanvas 的显示尺度为 `WEEK/MONTH/QUARTER/YEAR`，密度分别为 40/12/4/1.5 px/day。初始档位选择第一种内容宽度不超过四个时间区视口的尺度；用户选择后 Resize 不覆盖。视觉尺度不参与业务校验：Segment 拖选、移动和调整固定吸附 30 分钟，Composer anchor 固定吸附一个上海自然日。冻结行标题使用响应式 120-280px 范围，完整画布通过独立底部横向滚动条控制时间区，Dashboard 与投入详情使用 `COMPACT` 展示。部分确认在事务锁行后强制覆盖起点等于权威 Planned 起点，只创建 Actual 与最多一条尾部 Planned；直接拆分的公开 action、validation、service 和 capability 已移除，既有审计、来源和 notification outbox 语义不变。
