...
- Milestone Review 允许 OWNER/PARTICIPANT/全局管理员提交 TEXT/LINK 证据；FILE 证据当前返回中文校验错误。Termination Review 允许相同提交角色送审拟定 outcome、reason 和 summary，不新增证据。只有两类全局管理员可以通过、驳回或要求修订两类 Review，并允许自审。Milestone 通过后推进节点；Terminal 只有通过后才结束 Task。驳回和要求修订均保留历史、释放门禁并允许调用方使用新 `idempotencyKey` 修改后重提。Task Workspace 保持轻量；用户选中已完成 Milestone 时，会通过重新校验 Task 可见性和 Current Plan 节点归属的详情查询，按需读取 `MilestoneNode.completedAt` 及最终未撤销通过轮次的有序 Evidence，非 HTTP/HTTPS 链接不会下发为可点击地址；已完成 Terminal 直接使用 Workspace 中的 `confirmedAt/outcome/reason/summary` 展示实际结束事实。待审批 `currentOnly` 查询与历史计划/版本比较 DTO 不因详情展示而扩充材料正文。
