...
时间画布查询默认排除 `PLANNED + CONFIRMED/CANCELLED`，但不删除事实记录、来源和历史。PERSONAL scope 的 Task universe 来自有效 TaskMember；“我的时间”默认按 ACTIVE 状态、稳定游标每页装配 25 条参与 Task 及其 Plan 行，显式 `tasks=all` 才包含草稿和终态。未指定 `date` 时窗口从上海时区当天开始；日期平移 range bar 通过受控 `onRangeChange` 更新 URL，可向过去或未来移动。Task 详情以有效 TaskMember 为人员范围，Project 详情以 ProjectMember 与当前 Task 页 TaskMember 并集为范围；两者使用 URL 保存的 31 天上海自然日窗口，跨窗口定位会先更新 URL 再聚焦节点。部分确认在事务锁行后强制覆盖起点等于权威 Planned 起点，只创建 Actual 与最多一条尾部 Planned；直接拆分的公开 action、validation、service 和 capability 已移除，既有审计、来源和 notification outbox 语义不变。
