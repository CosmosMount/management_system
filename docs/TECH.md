...
- Segment 服务支持单条/批量 Planned 创建、Actual 创建、更新、批量移动、拆分、合并、取消、完整确认、部分确认、重关联和 Actual 逻辑删除。所有写操作都在事务内写 `WorkSegmentChange` 和 `DomainAuditEvent`，并通过 `expectedUpdatedAt` 执行乐观锁校验；批量写入保持单事务全成全败。需要转换状态的入口按固定 ID 顺序取得 `WorkSegment` 行锁，cron transition 使用带旧状态与时间条件的 guarded update，只有真实状态变化才写 change、audit 和 outbox，因此并发或幂等重试不会重复副作用。
