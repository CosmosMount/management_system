...
- **停用账号界面**：项目管理和采购详情仍允许读取授权范围内的历史事实，capability 与操作区统一只读。采购模块侧栏/移动抽屉不为停用账号展示“新建申请”；直接访问新建页会重定向看板，编辑页返回脱敏 404。已下线的工坊加工费路径对所有账号返回 404。
...
- Playwright spec 以文件首行 `// @playwright-project node-db|ui` 声明执行拓扑。共享分类器递归发现 `.spec.ts`、拒绝缺失/重复声明，并在配置生成任何 project `testMatch` 前扫描 Playwright 1.61.1 默认的 `**/*.@(spec|test).?(c|m)[jt]s?(x)` 文件名；除 `.spec.ts` 外的默认匹配形式会显式失败，不能被 topology 静默排除。TypeScript AST 只静态追踪 spec 直接从 `@playwright/test` 导入的 `test` binding（允许 import alias）：该 binding 的每次引用都必须是已批准 direct `test...()` API 的 root，所有 test/suite/hook 注册 callback 必须 inline；本地/容器/factory alias、computed/间接 test API 和 `test.extend` 都会拒绝。已批准 path 与仓库安装的 Playwright 1.61.1 `TestType` 保持一致，包括 `test.describe.fixme`、`test.describe.serial.only`、`test.describe.parallel.only` 及 `test.expect` 的 `soft`/`poll`/`configure`/`extend`/asymmetric matcher 入口；`test.info()`、configured/extended Expect 与 matcher 是明确的返回值边界，后续合法读取或调用不会被误判为 test binding alias。`test.skip`/`fixme`/`fail`/`slow` 的 conditional callback 必须 inline，并与 test/hook callback 一样参与 fixture 分类；fixture key 使用 AST 解码值，Unicode escape 不能隐藏浏览器 fixture，computed key 会 fail closed。模块或 suite 注册阶段只允许官方 test API、未被局部绑定遮蔽且参数中不含可调用本地绑定的 Node 内建调用及少量确定性全局调用；能接收 callback 的 safe-global path、Promise 或未解析构造器、本地注册 helper、非 Node 导入、namespace 解构、callback 型 factory、getter/解构/对象展开及 custom iterator 等隐式执行都 fail closed。此策略不声称跨模块分析 custom fixture，只是在无法证明绑定安全时拒绝，Stage 3A 再统一 UI fixture。分类器据此拒绝 node-db 中出现 `page`/`browser`/`context` fixture 或 UI 文件完全不使用浏览器 fixture。`node-db` project 无设备配置且每个 spec 只收集一次；UI spec 分别进入 `desktop`（1440×1000）和 `mobile`（Pixel 5）。默认 reporter 对 Playwright 实际 suite 始终复核分类和 project；校验失败时先把已收集 suite 全部标为 skipped，阻止测试体副作用，再由 `onEnd` 返回失败。`--project` 与 Playwright 1.61.1 一致按大小写不敏感的精确名称或 `*` wildcard 选择，parser 与 reporter 共享同一 project selection helper。只有文件/行号、grep、grep-invert、shard、last-failed、only-changed、test-list/test-list-invert 会进入 partial，其他受支持参数仍逐一要求所有与所选 project 相交的 topology 文件完整出现，未知长参数直接拒绝。`.node.ts` 不在 Playwright testMatch 中，只由受控 Node runner 执行一次
