...
- Revision 只允许基于当前 Current Plan 和匹配的 `RevisionNode.baseTaskLockVersion` 创建。`revisionAt` 是用户选择的事件时间；它不形成阶段、不参与 Milestone 严格递增，也不能关联 Segment。目标计划固定沿用 Current Start，自动保留全部已完成 Milestone 和已生效 Revision，并重建全部未完成 Milestone 与 Termination。创建即为 `PENDING_APPROVAL`；被驳回记录保留候选计划，修改时递增 `reviewRound` 并直接重新送审，不存在 `DRAFT` Revision 或单独 submit。每个 Task 只允许一个 `status=DRAFT` 的 Revision 候选计划。Participant 可管理自己创建的未生效 Revision，Owner/全局管理员可管理该 Task 任意未生效 Revision。只有全局管理员批准后才原子历史化旧 Current、启用新 Current 并标记被替换节点为 `REVISED`；Segment 仅关联 Task，因此无需关联失效或重关联流程。Task 工作台对 Current Plan 中的已生效 Revision 提供独立历史开关；专用查询使用 `taskId + revisionNodeId` 复核 `plan.view_history`、Current Plan 节点归属和基础 Plan 的 Task 归属，客户端首次勾选时按需读取 `basePlanVersion` 并缓存。选中的基础 Plan 以带唯一前缀的只读 Plan 行和中性历史状态 anchor 插入 Current Plan 与人员投入之间，不把后来变化的 `TaskNode.status` 表述为 Revision 当时的执行状态，也不复制 Segment。历史行仅作为最终 `TimeCanvas` 的 `presentationOverlay` 合并，权威 `initialModel`、数据块缓存、`rowPageKey`、Inspector 状态及服务端数据查询范围均保持不变；overlay 更新不会关闭已打开的投入详情或覆盖未保存输入。历史范围默认仍以 Current Plan 为中心裁剪到三年窗口；“最早内容 / 最新内容”只在客户端重定位 presentation 窗口，既不把历史中心或缩放写入权威 URL，也不为远期历史端点请求人员投入数据。外部节点聚焦、center 更新、浏览器前进后退或移除对应 overlay 会先退出历史窗口，再恢复权威中心。
