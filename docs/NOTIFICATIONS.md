...
- 内容为项目、Task、当前计划待办节点、负责人、计划启动/节点截止、逾期/未激活风险和里程碑验收、计划修订、任务结束三类待审批的 Markdown 表格，无事项时明确提示。完整快照保存在执行记录；每次仅投递一条不超过 6000 字符的摘要，超长内容保留完整表格行并明确提示到执行记录查看全文，避免大量分段消息。卡片按纯文本展示 Markdown 源文，不依赖飞书渲染表格，也不截成普通通知的 180 字摘要。
