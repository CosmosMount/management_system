...
- `meeting_work_segment_reminder` 由在职参会者手动发起，仅发送给选中的在职且绑定账号的本会议参会者，不附加管理员。未填写指没有与会议工作区间相交的未删除投入记录，不按自然周或填写次数判定。
- 使用现有 TASK 分类的强制站内通知与通知机器人 Outbox，不受该分类飞书偏好关闭影响，但保留全部禁发、允许名单和投递保护；无飞书身份者仍有站内通知。消息纯文本展示会议主题、北京时间工作区间、提醒人、行动说明和会议详情入口，不使用审批机器人。
- 审计、站内通知、Outbox 事务提交；会议级锁与请求标识保证同一请求重试不重复发送，新请求允许再次提醒。会议版本变化要求重新打开弹窗。历史保存实际选中对象和区间，投递结果由既有 Outbox 跟踪。

## 管理员全局进度总结

- 所有进度提醒卡片的“立即执行”均在开关左侧；只运行选中类别，不改启用状态或下一次定时时间。手动请求按操作账号、类别和 UUID 去重，失败重试复用原请求；再次独立执行使用新请求。原有提醒保持其收件规则，不因管理员发起而仅发给该管理员。
- 两类总结每个 Task 只列当前计划顺序中的第一个 ACTIVE 节点；没有进行中节点时明确标注，不展示后续 PENDING 节点。待审批事项独立汇总，不随节点展示筛选而遗漏。

- `project_management_global_summary_daily` 专供有效、无团队/技术组限制的 `SUPER_ADMINISTRATOR` 与 `PROJECT_ADMINISTRATOR`，不发普通成员或局部管理员。管理员执行入口与历史读取实时校验角色；飞书投递再次检查角色/人员状态，撤权后也无法读取原站内总结。
- 内容为项目、Task、当前计划待办节点、负责人、计划启动/节点截止、逾期/未激活风险和里程碑验收、计划修订、任务结束三类待审批的 Markdown 表格，无事项时明确提示。完整快照保存在执行记录；每次仅投递一条不超过 6000 字符的摘要，超长内容保留完整表格行并明确提示到执行记录查看全文，避免大量分段消息。飞书端参照报销明细使用 CardKit JSON 2.0：正文使用 markdown 元素，管道表格解析为原生 table 的 columns/rows，单元格使用 text 类型，最多展示 20 行、每页 10 行，超出明确提示查看全文；不再把表格源码交给 Markdown 渲染，也不截成普通通知的 180 字摘要。两类总结均走现有 CardKit 创建与发送链路，仍使用通知机器人、Outbox、收件权限和禁发保护。
- 使用通知机器人及现有 Outbox/投递保护；遵循普通 PROJECT 通知偏好，站内记录保留。自动事件键含上海业务日期、配置时间点；手动含操作人、requestId，通知保留固定 `part:1` 后缀。多个时间点分别汇总，重试相同请求不重复通知；不同手动请求可以再次发送。
- 管理入口：`/progress/notifications?view=settings` 的“管理员全局进度总结”。启用/禁用只影响自动调度，禁用时手动执行需确认；入队成功不等于送达成功，实际重试/送达状态由 Outbox 管理。
- 通知全文链接为 `/progress/notifications/summaries/<执行记录ID>`，可访问不在最近 20 条列表中的原始快照；详情查询将有效全局管理员条件与记录查询放在同一 SQL 中，撤权后不再返回正文。

## 个人进度总结

- `project_management_personal_summary_daily` 按每个在职且绑定账号的人员分别生成，不把所有人的总结群发给管理员。包含本人创建或仍参与的有效 Task；有效全局管理员另包含其待审批 Task。没有待办时发送明确的空状态总结。
- 复用普通 PROJECT 通知偏好、站内通知和通知机器人 Outbox，不追加超级管理员，也不使用审批机器人。每个人独立通知与全文记录，自动按上海日期/时间点去重，手动按独立请求去重。
- 每个 Task 只列首个进行中节点，审批独立汇总。全文仅对应在职账号可读取；包含管理员待审批信息时，撤权后禁止读取和后续飞书投递。完整 Markdown 存档，飞书仅发送有界摘要与本人专属详情入口。
...
下表列出原业务收件人；现行项目/任务运营事件会统一追加有效的全局超级管理员（`SUPER_ADMINISTRATOR`，全局范围、角色未撤销、关联 Person 为 ACTIVE），按账号去重。成员专属的任务分配/成员变化与项目成员加入通知只发送给实际受影响成员，不追加超级管理员；其他追加范围包括项目立项、信息更新、任务归属变化、结束/删除，任务更新、激活、删除/结束，里程碑今日到期/逾期及验收，计划修订待审批/结果/生效/取消，结束申请及结果，以及风险提出/解决和评论发布。仅有项目管理员角色的账号不因此新增订阅；原业务收件人不减少。新增超级管理员同样遵守普通飞书分类偏好，原强制事件仍强制，站内通知始终保留；缺少飞书身份不影响站内通知。普通事件没有有效超级管理员时不阻断业务，审批可用管理员校验保持不变。

该规则只作用于新产生的事件，不补发历史或改写已送达的站内/飞书记录；尚未投递的成员专属 outbox 会在投递前重新校验实际成员资格并取消错误收件人。账号安全仍仅通知被操作账号；零成员草稿创建仍不产生任务分配通知；普通投入增删改、评论删除及已退役事件不新增通知。成员变更等事件仍逐条创建站内记录和 outbox，符合上述条件时仅在飞书外发层按收件人与类别合成卡片；成员本人如果同时具备超级管理员身份仍只按同一账号去重接收一次。
...
| 在途订单停留催办 | `scheduled_reminder` | outbox 逐收件人私信 | 当前处理人 |
| 采购人手动催促 | `manual_reminder` 仅作限流哨兵 | 当前状态决定的私信；老师环节可附加邮件 | 当前处理人 |

采购日报与定时催办在每日 09:00 分别调度；日报查询或 Webhook 发送失败不会阻断催办扫描，各自记录失败日志。定时催办扫描只查询停留超过 24 小时且距上次入队超过 24 小时的订单，分批读取候选 ID；订单锁内复核状态、审批方和收件人，将本轮 `statusEnteredAt` 与日期槽写入稳定事件键，并在同一事务更新 `lastReminderAt`。该时间表示入队，不代表送达。outbox 按收件人保留成功和失败历史，只重试失败者；投递前重查订单轮次和当前角色，过期或撤权后取消未发消息。采购日报和手动催促仍通过统一飞书传输层或独立 Webhook 模块，继续受禁发开关、allowlist、机器人用途和日志脱敏约束，不能自行获取 token 或直接调用 IM API。
...
- `approval_urged` 用于里程碑验收、计划修订和任务结束申请的人工催促，不改变审批状态、顺序或截止时间。
- 发起人可在任务工作台选择当前有效审批人，默认全选；服务端提交时重新校验审批仍为待处理、目标仍在职且具备全局审批权限，客户端选择结果不作为授权依据。
- 催促通过站内通知和通知机器人发送，不使用审批机器人；继续遵循 Outbox、禁发保护、收件人状态校验和投递重试规则。
- 审批催促不设时间间隔限制，每次独立请求均可再次发送；保留相同 `requestId` 的重复请求检查。审计事件保存发起人、审批类型、审批记录和实际收件人快照。普通任务催促的 5 分钟冷却不变。
