...
总结汇总有效 Task 的首个进行中节点、负责人、计划启动/截止、未激活/逾期风险，以及验收、计划修订和任务结束三类待审批事项；个人总结按本人参与和待办筛选，全局总结仅通知有效全局管理员，不替代原有成员提醒。自动任务需要单独运行 `npm run cron`；升级时通过既有 `npm run db:deploy` 部署新迁移。本功能按桌面界面设计。
