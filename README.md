...
Next.js 全栈管理系统。采购报销、项目管理、物资管理与反馈中心共用飞书统一账号。

所有设备共用一套页面、导航、表格和时间线，不按设备隐藏业务功能或切换专用界面。窄窗口通过内容区域滚动、时间线缩放及通用表单访问完整功能。

时间线的任务行与计划轨道统一显示“项目名称 / 任务名称”，两个名称分别链接至项目和任务详情；无所属项目时仅显示任务名称。未保存的新任务不提供任务详情链接，人员行保持原样，状态与进度警示不变。

进行中的任务详情在“修改任务基本信息”左侧提供“催促任务”；所有有查看权限的有效登录用户均可使用，无需编辑权限。催促信息可不填，最多 500 字；同一任务所有用户共享 5 分钟冷却。催促向当前有效负责人、参与人、两类全局管理员及催促人生成站内通知，并通过通知机器人发送强制普通飞书通知（不受 TASK 分类偏好关闭影响，但遵守禁发、白名单和有效身份规则）。成功提示表示已提交投递，不代表飞书已送达；催促记录会显示在近期动态中，不改变任务或审批状态。

编辑已有任务草稿并保存成功后，系统会询问是否立即激活。选择“暂不激活”返回任务详情并保留草稿；选择“立即激活”仍由服务端校验权限、最新版本和激活条件。激活失败不回滚已经保存的修改；网络异常时应返回详情确认最新状态。新建草稿和计划修订不显示此提示。

有激活权限的用户离开草稿任务详情（点击当前标签页链接或浏览器返回）时，会再次收到激活提醒，可留在当前页、暂不激活继续离开，或激活成功后离开。无激活权限和非草稿任务不提示；进入当前任务的编辑页、同页时间线定位和新标签页链接不提示。刷新或关闭标签页只能显示浏览器原生离页确认，不能在其中激活任务；实际激活仍由服务端校验权限、版本和计划条件。
...
手机与电脑使用相同网址、账号权限和业务流程。宽度不足 1024px 时，顶部菜单打开系统导航，模块标题旁的菜单打开采购、物资或项目导航；全部入口仍可访问。普通列表在窄屏重排为卡片，采购汇总等比较型表格保留区域内横向滚动。

- 时间轴可用手指滑动浏览；选择投入后使用“编辑投入／查看投入详情”。较短或重叠记录可从“当前时间范围的投入”列表选择，每次展开 20 条。触摸滑动不会直接修改时间。
- 任务计划可在“勾选节点与批量移动”中勾选节点、填写天数并整体移动，也可直接编辑节点时间；保留原有边界校验和撤销。桌面仍支持鼠标拖动和快捷键。
- 反馈在手机显示列表或当前会话，使用“返回反馈列表”切换；当前页面内切换会话或旋转屏幕会保留各会话的未发送文字和图片，离开或刷新页面不保证保留。
- 手机可使用现有二维码链接领用、归还物资并上传照片。D110 Type-C 直连打印仍要求浏览器支持 Web Serial；不支持时可下载二维码或复制链接到支持的电脑处理。

实施清单与实际验收记录见 [`docs/MOBILE_INTERFACE_PLAN.md`](docs/MOBILE_INTERFACE_PLAN.md)。窄屏与触摸自动化不代表已完成 iOS、Android 或飞书内置浏览器真机认证。

## 管理员全局进度总结

系统管理员在“项目管理 → 通知 → 通知设置”（`/progress/notifications?view=settings`）配置各类每日提醒、“个人总结”和“管理员全局总结”。每类支持多个时间点及启用/禁用；卡片右上角开关左侧的“立即执行”经确认后仅运行该类提醒一次，不改变定时开关，也不占用下一次定时执行。个人总结分别发给相关人员，全局总结只发有效全局管理员；两类总结每个 Task 只报告当前计划中按顺序排列的第一个进行中节点，没有进行中节点时明确标注，不列出后续未开始节点。管理员全局总结保留最近 20 次执行记录及完整 Markdown。

总结汇总有效 Task 的首个进行中节点、负责人、计划启动/截止、未激活/逾期风险，以及验收、计划修订和任务结束三类待审批事项；个人总结按本人参与和待办筛选，全局总结仅通知有效全局管理员，不替代原有成员提醒。自动任务需要单独运行 `npm run cron`；升级时通过既有 `npm run db:deploy` 部署新迁移。配置入口沿用全站响应式界面。

## 会议记录

- 全局超级管理员可在会议列表的“会议模板”区域创建、编辑和删除共享模板，其他用户不展示模板。模板包含名称、说明及会议主题、参与人、展示项目、展示任务、纪要，不包含任何工作时间或周期配置。
- 从模板卡片“使用模板”，或在创建会议页选择模板，会一次性填充非时间字段，不会立即创建会议。新建会议仍默认最近七天；更换模板需确认覆盖已有非时间内容（模板空值也会覆盖），始终保留已填时间。关闭模板提示不清空内容。
- 填充后可自由修改；会议与模板无关联，修改或删除任意一方都不影响另一方。模板删除保留审计历史，不发送通知。引用的人员或项目／任务已不可用时显示字段错误，需重新选择后保存；模板本身被删除不影响已填好的有效会议草稿。
- 模板列表按更新时间倒序独立分页，不受会议筛选影响。`20260913140000_meeting_templates` 仅新增模板及参与人表，不回填现有会议；部署前按测试指南在隔离 PostgreSQL 验证迁移。

- 在职参会者可在详情页“相关任务进度”内发送投入填写提醒。弹窗默认选择会议工作区间内没有未删除投入记录的人员（以区间相交判断），也可补选其他参会者；停用或未绑定账号者不可发送。发送生成站内通知和普通通知机器人队列，不代表飞书已送达。同一请求重试不重复发送，新一次提醒不设冷却时间。

- 会议列表支持主题、参与人（含历史会议中的停用人员）、我创建的会议、直接展示项目/任务及未关联条件组合筛选。工作区间可选最近 7/30/90 天开始，或自定义北京时间日期范围（含结束日期当天，按区间相交筛选）；这不是会议召开日期。默认按创建时间倒序，也可按最近更新或工作区间开始排序。筛选条件保存在 URL 中，翻页保留条件，重新筛选回到第一页，“重置”恢复默认。

- 创建或编辑会议时可选“展示项目”和“展示任务”（各最多 50 个），仅保存时间线展示配置，不改变项目归属或参与人。所选项目动态展示其当前任务，直接选择的任务移动项目后仍展示；重复选择自动合并。
- 时间线包含所选任务的当前计划、节点及工作范围内所有人员的投入；不要求任务已有工作记录，也包含已完成／归档任务。原数据变化在重新打开或刷新时间线时体现。不可用对象提示管理员移除，不影响其余内容；纪要保持普通多行文本。
- 会议详情、创建及编辑预览复用工作台完整时间线：默认周尺度、底部滑块、今天定位和双击投入只读详情。日历范围按内容留白；超长计划复用三年显示窗口及最早／最新内容定位，不扩大会议工作记录查询范围，不引入额外日期输入或 31 天切换。
- `20260912150000_meeting_timeline_display` 为已有会议增加空展示配置，保留原纪要和版本；需先在隔离库验证迁移，再部署应用。

项目管理主导航中的“会议”与“项目”并列，入口为 `/progress/meetings`，会议不归属于项目。

- 所有登录用户均可查看全部会议，包括主题、参与人、纪要及参与人的跨项目工作时间线；参与人不是访问白名单。
- 只有在职全局超级管理员可以创建、修改会议和纪要，项目管理员没有会议维护权限。保存即公开，纪要可以稍后补充；保存本身不发送通知、不设置草稿或发布流程。
- 创建时选择 1–50 位参与人及工作开始、结束时间，并预览现有时间线。时间为北京时间，工作区间采用 `[开始, 结束)`，最长 366 天，表示讨论的工作范围而非会议召开时间。
- 时间线显示实时数据，不保存历史快照；加载会议设置的完整工作时间范围，使用现有底部滑块和缩放功能浏览，不另设查看区间。单次查询超过 5000 条记录时明确报错，不静默省略。所有查看者均只读，点击记录可看详情。
- 纪要为可选的多行正文。多人编辑发生冲突时保留当前输入并提示刷新，不自动覆盖；已选参与人停用后仍保留历史关联。
- 会议详情的“导出会议纪要”供在职登录用户一键生成并复制纯 Markdown 文本，可粘贴到飞书文档继续记录。内容包括会议名称、北京时间工作区间（不是开会时间）、参与人和会议链接；进行中任务仅取指定任务及指定项目下的当前进行中任务，项目和任务分别链接；个人汇报填入参会人在完整工作区间内可查看的投入原文，“其他”保留已保存纪要，“下周安排”只列姓名待补充。无记录明确提示，不推测完成状态；剪贴板不可用时可在弹窗全选手动复制。导出读取最新数据、不修改会议、不下载文件或发送飞书消息，也不保证飞书自动将 Markdown 转为富文本。

新增数据库结构由 `20260912120000_independent_meeting_records` 迁移创建。部署先完成隔离库验证，再执行受控 `npm run db:deploy` 并重新生成 Prisma Client／重启应用。迁移只创建 `MeetingRecord`、`MeetingRecordParticipant`，不读取、覆盖或删除其他实验会议表；旧实验分支留下的迁移历史和数据应另行核对，不使用 reset 或手工修改迁移记账消除差异。
...
| `PROJECT_MANAGEMENT_NOTIFICATION_AGGREGATION_WINDOW_SECONDS` | 项目管理普通飞书私信聚合窗口，默认 `30` 秒，允许 `0-300`；设为 `0` 可恢复逐条投递 |
...
进入 `/materials` 查看物资台账。桌面端使用左侧栏，移动端使用导航抽屉；台账会直接显示每件物资的名称、价格、技术组、使用状态、当前使用人和领用时间。在职统一账号可通过“登记物资”录入名称、价格和所属技术组，系统随即为该物资生成长期唯一且不可修改的二维码，可在详情页下载并粘贴到实物上；停用账号只能查看历史台账。

登记物资支持输入名称与数量（单次 1–100 件，默认 1 件）。数量为 1 时保留原名；例如名称“示波器”、数量 3，会创建“示波器-1”“示波器-2”“示波器-3”三件独立物资，共用所填的单件价格与所属技术组，各自具有独立二维码、使用状态和审计记录。批量登记在同一事务中完成，重复提交同一请求不会重复创建；成功后返回物资台账，单件登记仍进入详情页。不同批次允许同名，不自动接续编号。

登记时可启用“配套物品”，分别填写第二种物品的名称、单价和技术组，两种物品共用数量并按相同序号组成一套。例如“无线麦克”和“接收器”各 3 件，会形成“无线麦克-1 + 接收器-1”等 3 套。扫描任一件都会原子领用或归还整套；任一件已被占用时整套不可领用。归还时拍摄一张包含整套物资的照片，系统为两件物品分别保留归还记录。配套关系不可修改，删除任一件会同时删除整套。

物资详情页支持“删除物资”：仅在职登记人或全局超级管理员可操作，使用中的物资必须先归还；配套物资会整套删除。二次确认后软删除，物资从台账和数量统计中移除，原二维码不可再领用；原详情地址保留只读信息、领用历史和审计，不提供恢复入口。删除不会清除物资、领用记录或现存归还照片，也不会发送通知。

扫码会打开物资确认页，不会在 GET 请求、相机预览或聊天软件链接预取时直接改变状态：

1. 物资可用时，当前登录用户确认后领用。
2. 该用户必须再次扫描同一二维码，现场拍摄物资当前状态并确认后归还；照片最大 8MB，支持 PNG、JPG、WebP、HEIC 和 AVIF。
3. 物资已由其他用户领用时显示当前使用人并拒绝操作，只有领用人可以归还。

归还照片只保留到下一位用户成功领用该物资：领用事务会先解除上一张照片与历史记录的关联并登记清理任务，随后删除文件释放空间；即时删除失败时由后台任务重试。领用/归还时间和领域审计永久保留。并发扫码由服务端行锁和数据库唯一约束串行化，重复提交使用同一请求标识幂等返回，不会把一次领用误执行成紧接着归还。

物资详情页支持 NIIMBOT D110 与 D110_M USB 直连打印；物资台账还可勾选当前筛选结果中的多件物资，全选或清空后连接一次打印机，并按台账顺序逐张打印标签。打印由打开页面的桌面版 Chrome/Edge 通过 Web Serial 直接访问该电脑连接的打印机，不经过服务器或本地打印服务；首次打印需在浏览器设备选择窗口中授权串口。局域网客户端必须通过 HTTPS 访问系统（`localhost` 仅适用于服务运行电脑），否则浏览器不会开放 Web Serial。标签使用 240×96 像素画布，包含物资二维码、名称、技术组和价格；连接后会校验设备型号，并根据 D110、D110_M 或 D110_M V4 自动选择相应打印协议。

### 完整审批与报销流程

进入 `/procurement` 会直接打开采购看板。通过统一左侧栏在「采购看板、待办与最近、新建申请、订单列表」之间切换；订单详情和编辑页归属“订单列表”。采购各面板顶部统一使用与项目管理一致的上下文命令栏，不显示返回按钮，状态和业务操作集中在命令栏右侧。独立“工坊加工费”录入入口已下线，旧 `/procurement/workshop-fee` 返回 404；普通采购申请仍可选择“加工费”种类，每条加工费明细可一次或分批上传最多 9 张参考图片，既有工坊加工费历史订单仍保留并可按原权限查看。停用账号仍可从“订单列表”读取自己的历史订单，但侧栏不显示新建申请，直达写入页面会进入只读或无权访问状态。
...
在「待申请人上传凭证」（含退回重传）或「待报销员处理」阶段，在职采购申请人可在「修改凭证」弹窗中逐张标记删除已上传发票，保存前可撤销，关闭弹窗不生效。保存时统一删除并追加新发票，最终须保留 1～20 张；可用新发票替换最后一张旧发票。报销员上传截图、进入「待申请人确认」后不能删除发票，仍可按原流程补充上传。已保存的删除没有恢复入口，旧下载链接不再提供文件。
...
每次发布前端变更（包括回滚发布）前，更新 `lib/frontend-version.ts` 中的 `FRONTEND_VERSION`，格式为 `年.月.日.修订号`，不要复用旧版本号。该版本将句点替换为连字符后用于 Next.js `deploymentId`（例如 `2026-09-11-2`），使新版本静态资源使用不同地址。更新代码后重新构建并重启应用；页面底部可查看前端版本和手动检查更新。

页面加载、重新聚焦／恢复可见及可见期间每分钟检查一次版本。发现新版本且当前没有操作时，客户端请求清理本站资源缓存后整页刷新，保留路径、筛选参数和锚点；有输入、按钮操作或弹窗时先提示保存，再由用户确认刷新。不会清 Cookie、Local Storage 或草稿，异常响应／断网不触发刷新，并有重复刷新保护。浏览器清缓存指令取决于安全上下文及浏览器支持，不能等同于清空浏览器全部缓存；不支持时仍依靠版本化资源地址和带版本标记的整页导航更新。首次接入此机制前已打开的旧页面需要手动刷新一次。
...
项目与任务列表的负责人列统一使用叠放圆形头像，只显示未移除的负责人（OWNER），不以参与人补位。展示前两位头像，超过两位以 `+N` 表示；无头像或加载失败时显示姓名首字，无负责人时显示 `—`。悬浮或键盘聚焦可查看完整负责人姓名，停用人员在提示中标注“已停用”；同名人员仍按不同人员保留。
...
提示覆盖工作台、任务列表与详情、项目任务概览、个人日程、人员时间线、资源计划、相关待办和任务选择器。页面可见时每分钟更新，恢复可见或重新聚焦时立即更新，不为颜色变化轮询数据库。工作台对全部参与任务按“逾期、临期、距到期超过 3 天、无有效到期目标”排序后完整展示，同类按截止时间、名称和 ID 排序；其他列表分页、待办优先级和通知规则保持不变。画布的到期文字和底色独立于节点图标、完成状态及修订计划配色。

工作台 `/progress` 按“工作指标 → 完整个人时间线 → 我的待办与参与任务双栏 → 折叠通知”排列，窄屏改为纵向排列；顶部指标在宽屏四列、中屏两列、手机单列，存在紧急待办时突出提醒。“我的待办”汇总任务推进与审批事项；参与任务表完整展示当前筛选范围内的名称、状态、当前节点和截止时间、计划版本，保留风险排序及颜色警示，切换“显示全部／只看进行中”与时间线同步并保留视口。表格仅在自身容器内横向滚动。工作台仅保留一个扁平页面，不再提供“我的工作／个人日程／管理概览”子视图导航；旧 `view=schedule`、`view=management` 链接统一回到工作台，保留有效的任务筛选、视口和投入定位参数。项目与任务列表提供单一搜索、范围、状态工具栏（任务另有优先级）和重置入口，原默认“我参与 + 进行中”不变；任务列表保留长内容展开，项目列表通过悬浮或聚焦提示查看完整名称与简介。

项目列表 `/progress/projects` 每行一个 Project，桌面依次展示项目、任务概览、汇总、项目状态、负责人、更新时间和打开入口，不嵌入 Node/Timeline。任务概览只显示草稿和进行中的 Task，按当前节点期限紧急优先排序，最多两行；超出部分以 `+N 个任务` 打开可滚动弹层，查看全部草稿和进行中任务并进入 Task 详情。Chip 保留真实状态，期限红／橙／绿沿用当前节点规则，无有效期限为中性色；长名称支持鼠标和键盘提示。项目整体完成进度为“已完成 / 所有未删除且非取消任务”，失败、超时、归档仍计入分母并单独统计；期限预警与进行中计数重叠，不相加。无草稿或进行中任务时仍显示整体完成进度。项目自身状态、负责人和更新时间与 Task 信息分开；项目顺序仍按更新时间（搜索时按相关性），筛选与游标分页不变。

旧 Project/Stage 工作流已清理；当前重新提供轻量 Project 文件夹和立项流程，不恢复 Stage、周报或旧审批角色。`/progress/projects` 提供默认“只看我参与 + 进行中”的列表、创建、详情、编辑、审批、驳回重提、结束和软删除。Project 顶部显示详情标题和说明，概览卡片展示头像、项目名称、完整描述、负责人、参与人员与任务完成进度；Task 概览卡片展示任务名称、完整描述与八项计划及成员信息。资料默认展开，操作保留在卡片右上角，当前节点到期警示与原有权限规则不变。Project 与 Task 工作台均采用同页布局：上方完整时间线与人员投入，下方桌面三栏（左侧风险与讨论、中间主体操作、右侧近期动态）；窄屏依次展示时间线、主体操作、风险讨论和近期动态。Task 默认选中当前节点，节点导航与审批详情同页联动；Project 的任务列表与时间线同页联动。旧 `section` 链接仍可打开完整工作台，不再控制分区显隐；保留 `focus`、`center`、缩放参数、风险与节点详情锚点以及刷新、前后退定位。当前节点到期警示继续在任务摘要、节点详情/导航、Project 任务列表及对应时间线节点显示：已逾期为红色，距截止不超过 72 小时为琥珀色，超过 72 小时为绿色，并配文字与图标；非进行中任务或无有效截止时间不显示警示。Project 主体详情把 Task 按七种状态分组，各组可独立折叠并可整组或逐条选择计划轨道；默认只展开并显示进行中 Task，折叠不改变已经选择的轨道，选择只存在于当前页面且不影响人员投入。Task 主体包含待处理 Revision、选中节点详情及风险录入，Project 主体包含任务列表及风险录入；审批门禁及全局反馈在时间线上方持续可见。等待审批的 Revision 会自动在 Current Plan 后加入只读、琥珀色的修改后候选 Plan；其后才是用户勾选的历史 Plan 与人员投入。Task 节点导航中的每条已生效 Revision 可独立勾选“显示修订前计划”，按需加入对应基础 Plan 的只读历史行；可同时比较多条，取消勾选只隐藏该行。对比内容跨越三年展示上限时，可用“最早内容 / 最新内容”在本地切换展示窗口，不会为对比端点请求人员投入。右侧近期动态继续把 `DomainAuditEvent` 格式化为可筛选的中文活动记录。Project 风险明确分为自身风险和当前所属 Task 风险；Project 评论不混入 Task 评论。立项申请与完整审计历史继续持久化；详情不恢复历史卡片，只在立项审批中展示当前轮的提交人、提交时间和申请加入的 Task。一个 Task 最多属于一个 ACTIVE Project；Task 加入时会把有效 Task 成员补为 Project Participant，但 Project 身份不授予 Task 权限。
...
- `/progress` 是“我的工作”统一驾驶舱，默认依次展示工作指标、完整时间画布、行动待办与完整参与任务表格、折叠通知，不再拆分子视图。行动待办直接展示前 8 项及其项目、任务、节点和时间信息；投入只通过时间线详情维护，没有到期确认队列。
...
- 投入统一为“人员 + 起止时间 + 工作内容 + 可选任务关联”。允许过去、当前、未来及重叠时间，单条最长 31 天、内容最多 2,000 字；没有计划/实际、状态、优先级、输出或确认。保留单条新增、编辑、软删除和拖动/边界调整，也可通过表单维护；不再提供批量、拆分、合并或确认流程。权限与任务关联限制不变，操作只记录审计，不产生投入通知。
