...
物资详情页支持 NIIMBOT D110 与 D110_M USB 直连打印。打印由打开页面的桌面版 Chrome/Edge 通过 Web Serial 直接访问该电脑连接的打印机，不经过服务器或本地打印服务；首次打印需在浏览器设备选择窗口中授权串口。局域网客户端必须通过 HTTPS 访问系统（`localhost` 仅适用于服务运行电脑），否则浏览器不会开放 Web Serial。标签使用 240×96 像素画布，包含物资二维码、名称、技术组和价格；连接后会校验设备型号，并根据 D110、D110_M 或 D110_M V4 自动选择相应打印协议。

### 完整审批与报销流程

进入 `/procurement` 会直接打开采购看板。通过统一左侧栏在「采购看板、待办与最近、新建申请、订单列表」之间切换；订单详情和编辑页归属“订单列表”。采购各面板顶部统一使用与项目管理一致的上下文命令栏，不显示返回按钮，状态和业务操作集中在命令栏右侧。独立“工坊加工费”录入入口已下线，旧 `/procurement/workshop-fee` 返回 404；普通采购申请仍可选择“加工费”种类，既有工坊加工费历史订单仍保留并可按原权限查看。停用账号仍可从“订单列表”读取自己的历史订单，但侧栏不显示新建申请，直达写入页面会进入只读或无权访问状态。
