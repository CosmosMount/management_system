...
| `FEISHU_DIRECT_MESSAGE_ALLOWED_NAMES` / `FEISHU_DIRECT_MESSAGE_ALLOWED_OPEN_IDS` / `FEISHU_DIRECT_MESSAGE_ALLOWED_UNION_IDS` | 可选，飞书私信收件人临时 allowlist；用于测试或演练防误发，未配置时不限制。Playwright 启动的应用服务默认只允许 `李棋轩`；Docker Compose 默认禁用真实投递且只放行 `李棋轩`，生产真实投递需显式设置 `NOTIFICATION_DELIVERY_DISABLED=false` |
