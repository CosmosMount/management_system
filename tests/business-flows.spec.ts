...
test("项目阶段风险可通过项目详情同步、总览并取消", async ({
  page,
  context,
  baseURL,
}) => {
  await loginAsNormalUser(context, baseURL, normalAuth);
  await page.goto(
    `/progress/${fixtures.projectId}?stage=${fixtures.projectCurrentStageId}`,
    { waitUntil: "networkidle" },
  );
  await expect(page.getByText("PW全功能-逾期项目")).toBeVisible();
  const overview = page.getByTestId("project-risk-overview");
  await expect(overview.getByText("项目风险总览")).toBeVisible();
  await expect(overview.getByText("PW全功能-活动风险")).toBeVisible();

  const riskNote = `PW全功能-阶段风险-${Date.now()}`;
  await page
    .getByPlaceholder("说明阶段风险、阻塞或需要组长/项管介入的问题")
    .fill(riskNote);
  await page.getByRole("button", { name: "同步阶段风险" }).click();

  await expect
    .poll(async () => {
      const [stage, risk, outbox] = await Promise.all([
        prisma.projectStage.findUniqueOrThrow({
          where: { id: fixtures.projectCurrentStageId },
          select: { riskNote: true },
        }),
        prisma.projectStageRiskRecord.findFirst({
          where: {
        ...
        prisma.notificationOutbox.findFirst({
          where: {
            ...
            payload: { contains: riskNote },
          },
          select: { payload: true },
        }),
      ]);
      return {
        stageRiskNote: stage.riskNote,
        riskCreated: !!risk,
        outboxHasRecipients: outbox?.payload.includes("recipientOpenIds") ?? false,
      };
    })
    .toEqual({
      stageRiskNote: riskNote,
      riskCreated: true,
      outboxHasRecipients: true,
    });

  const createdRisk = await prisma.projectStageRiskRecord.findFirstOrThrow({
    where: { stageId: fixtures.projectCurrentStageId, content: riskNote },
    select: { id: true },
  });

  await expect(overview.getByText(riskNote)).toBeVisible();
  await expect(page.getByText("同步了阶段风险")).toBeVisible();

  const resolveNote = `PW全功能-阶段风险已取消-${Date.now()}`;
  await page.getByPlaceholder("风险取消说明").fill(resolveNote);
  await page.getByRole("button", { name: "取消风险" }).click();
  ...
  await expect
    .poll(async () => {
      const [stage, risk, outbox] = await Promise.all([
        ...
        prisma.projectStageRiskRecord.findUniqueOrThrow({
          where: { id: createdRisk.id },
  ...
  await expect(page.getByText("取消了阶段风险")).toBeVisible();
  await expect(page.getByText("已取消风险（1）")).toBeVisible();
  await expectHealthyPage(page);
  ...
  await expect(page.getByRole("button", { name: "同步阶段风险" })).toHaveCount(0);
  await expectHealthyPage(page);
});

test("已完成阶段不可新增风险但可取消已有风险", async ({
  page,
  context,
  baseURL,
}) => {
  const stage = await prisma.projectStage.findFirstOrThrow({
    where: { projectId: fixtures.completableProjectId, status: "COMPLETED" },
    select: { id: true },
  });
  const riskNote = `PW全功能-完成阶段遗留风险-${Date.now()}`;
  const risk = await prisma.projectStageRiskRecord.create({
    data: {
      stageId: stage.id,
      content: riskNote,
      source: "MANUAL",
  ...
  await prisma.projectStage.update({
    where: { id: stage.id },
    data: { riskNote, riskUpdatedAt: new Date() },
  });

  await loginAsNormalUser(context, baseURL, normalAuth);
  await page.goto(`/progress/${fixtures.completableProjectId}?stage=${stage.id}`, {
    waitUntil: "networkidle",
  });

  await expect(page.getByTestId("project-risk-overview").getByText(riskNote)).toBeVisible();
  await expect(page.getByRole("button", { name: "同步阶段风险" })).toHaveCount(0);
  const resolveNote = `PW全功能-完成阶段风险取消-${Date.now()}`;
  await page.getByPlaceholder("风险取消说明").fill(resolveNote);
  await page.getByRole("button", { name: "取消风险" }).click();

  await expect
    .poll(async () => {
      const [updatedStage, updatedRisk] = await Promise.all([
        prisma.projectStage.findUniqueOrThrow({
          where: { id: stage.id },
          select: { riskNote: true },
        }),
      ...
      return {
        stageRiskNote: updatedStage.riskNote,
        riskStatus: updatedRisk.status,
        resolveNote: updatedRisk.resolveNote,
      };
    })
    .toEqual({
      stageRiskNote: "",
...
test("项目负责人可从验收标准 CSV 批量导入任务且只发送汇总通知", async ({
  ...
  await selectImportedTaskAssigneeIfNeeded(page, dialog, normalAuth.openId, "李棋轩");
...
test("普通用户提交立项后创建立项中项目且不能直接启动", async ({
  ...
  await selectUserFromSearch(page, "搜索项目负责人", normalAuth.openId, "李棋轩");
...
test("立项被驳回后可基于原项目修改并重新提交", async ({
  ...
  await page.getByText("项目名称").locator("xpath=following::input[1]").fill(projectName);
  await selectUserFromSearch(page, "搜索项目负责人", normalAuth.openId, "李棋轩");
  await selectUserFromSearch(
    page,
    "搜索参与人员",
    fixtures.adminOpenId,
    "Playwright 管理员",
  );
  await page.getByText("车组").locator("xpath=following::button[1]").click();
  await page.getByRole("option", { name: "英雄" }).click();
...
async function fillTaskCreationRequestDialog(page: Page, title: string) {
  ...
  await dialog
    ...
    .fill("PW全功能-任务创建申请说明");
  await dialog.getByLabel("宣运").check();
  await selectUserFromSearch(page, "搜索负责人姓名", normalAuth.openId, "李棋轩");
  await dialog
    .getByText("定量/定性指标")
    .locator("xpath=following::input[1]")
    .fill("PW全功能-创建申请指标");
...
async function selectUserFromSearch(
  ...
  await page.getByPlaceholder(placeholder).nth(index).fill(searchText);
  await clickUserSearchOption(page, searchText, optionName);
}
...
async function selectImportedTaskAssigneeIfNeeded(
  ...
  await detail.getByPlaceholder("搜索负责人姓名").fill(openId);
  await clickUserSearchOption(page, openId, name);
}

async function clickUserSearchOption(
  page: Page,
  openIdOrSearchText: string,
  optionName: string,
) {
  const byOpenId = page.locator(
    `[data-testid="user-search-option"][data-open-id="${cssAttributeValue(openIdOrSearchText)}"]`,
  );
  if ((await byOpenId.count()) > 0) {
    await expect(byOpenId).toHaveCount(1);
    await byOpenId.click();
    return;
  }
  ...
  expect(
    await byName.count(),
    `用户搜索候选项 "${optionName}" 不唯一，请改用 openId 选择`,
  ).toBe(1);
  await byName.click();
}
