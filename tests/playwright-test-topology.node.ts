...
test("当前 Playwright spec 唯一归入 42 个 UI 或 53 个 node-db", () => {
  const topology = discoverPlaywrightTestTopology();
  assert.equal(topology.ui.length, 42);
  assert.equal(topology.nodeDb.length, 53);
  assert.equal(new Set([...topology.ui, ...topology.nodeDb]).size, 95);
  assert.ok(
