...
test("desktop task composer reveals invalid fields and preserves a recovered zero-milestone draft", async ({ context, page, baseURL }, testInfo) => {
  ...
  const taskName = page.getByRole("textbox", { name: "Task 名称*", exact: true });
  await page.getByRole("button", { name: "创建 Task 草稿", exact: true }).click();
  await expect(taskName).toBeFocused();
  await expect(taskName).toHaveAttribute("aria-invalid", "true");
...
test("desktop draft edit retains advanced fields and refuses a stale save", async ({ context, page, baseURL }, testInfo) => {
  ...
  await expect(page.getByRole("button", { name: "保存 Task", exact: true })).toBeDisabled();
  await openTaskComposerDisclosure(page, "补充说明与优先级");
  ...
  await page.getByRole("button", { name: "保存 Task", exact: true }).click();
  await expect(page.getByText("Task 已在服务端更新，当前本地修改不会覆盖最新版本。请先导出，或放弃并加载最新版本。")).toBeVisible();
  await expect(page.getByLabel("描述", { exact: true })).toHaveValue(description);
...
test("desktop revision creation and resubmission retain read-only baselines and approval semantics", async ({ context, page, baseURL }, testInfo) => {
  ...
  await expect(page.getByRole("textbox", { name: "Task 名称*", exact: true })).toBeDisabled();
  ...
  await page.getByRole("button", { name: "创建并送审", exact: true }).click();
  await expect(page.getByLabel("Revision 名称")).toBeFocused();
  const reason = `Stage4 候选计划 ${randomUUID()}`;
  await page.getByLabel("Revision 名称").fill(reason);
  await page.getByLabel("Revision 详细内容").fill("明确后续里程碑验收材料，不覆盖已承接节点");
  ...
  await expect(page.getByRole("textbox", { name: "Task 名称*", exact: true })).toBeDisabled();
  await page.getByLabel("Revision 详细内容").fill("已补充验收材料与负责人复核步骤");
  await page.getByRole("button", { name: "修改并重新送审", exact: true }).click();
...
test("desktop project upload errors focus the field and establishment remains pending", async ({ context, page, baseURL }, testInfo) => {
  test.setTimeout(90_000);
  const errors = collectPageErrors(page);
  const fixture = await createUiFixture();
  await loginAsTestUser(context, baseURL, { openId: fixture.owner.openId, name: fixture.owner.person.displayName });
  await page.goto("/progress/projects/new");
  await expect(page.getByLabel("上传头像", { exact: true })).toBeAttached();
  await expect(page.getByRole("combobox", { name: "搜索可加入的 Task", exact: true })).toBeVisible();
  await desktopEvidence(page, errors, testInfo, "project-create-first");
  await page.getByRole("button", { name: "提交立项", exact: true }).click();
  await expect(page.getByLabel("Project 名称")).toBeFocused();
  const name = `Stage4 项目渐进披露 ${randomUUID()}`;
  await page.getByLabel("Project 名称").fill(name);
  await page.getByLabel("Project 内容").fill("基础资料与项目负责人先填写，可选头像和关联 Task 按需选择。");
