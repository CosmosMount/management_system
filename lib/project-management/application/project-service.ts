...
export async function completeProject(actor: ProjectManagementActor, input: unknown): Promise<ProjectMutationResult> {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await notifyLifecycleTx(tx, refreshedActor, project, "project_completed", "Project 已结束", `Project「${project.name}」已结束`, updated.lockVersion, tasks.length);
    return { projectId: project.id, status: updated.status, lockVersion: updated.lockVersion };
  });
}
...
export async function deleteProject(actor: ProjectManagementActor, input: unknown): Promise<ProjectMutationResult> {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await notifyLifecycleTx(tx, refreshedActor, project, "project_deleted", "Project 已删除", `Project「${project.name}」已删除，关联 Task 已保留并移出 Project`, updated.lockVersion, taskIds.length);
    return { projectId: project.id, status: updated.status, lockVersion: updated.lockVersion };
  });
}
...
async function replaceProjectMembersTx(tx: PrismaTx, project: ProjectForMutation, actor: ProjectManagementActor, requested: ProjectMemberInput[], targetProjectName: string) {
  ...
  if (additions.length) {
    ...
    for (const member of additions) {
      ...
      await createProjectManagementEventNotificationsTx(tx, { actor, project: { id: project.id, name: targetProjectName }, kind: "project_member_added", category: "PROJECT", eventKey: `pm:project:${project.id}:member:${member.personId}:${member.role}:${project.lockVersion + 1}`, title: "你已被加入 Project", summary: `你已作为${member.role === "OWNER" ? "负责人" : "参与人"}加入 Project「${targetProjectName}」`, entityType: "ProjectMember", entityId: member.personId, linkPath: `/progress/projects/${project.id}`, mandatory: false, recipients, context: { role: member.role } });
    }
  }
}
...
async function notifyEstablishmentSubmittedTx(tx: PrismaTx, actor: ProjectManagementActor, project: { id: string; name: string }, requestId: string, round: number) {
  ...
  await createProjectManagementEventNotificationsTx(tx, { actor, project, kind: "project_establishment_submitted", category: "PROJECT", eventKey: `pm:project:${project.id}:establishment:${round}:submitted`, title: "Project 立项待审批", summary: `Project「${project.name}」第 ${round} 轮立项申请等待审批；负责人：${details.ownerNames.join("、") || "未设置"}；申请纳入 ${details.taskCount} 个 Task`, entityType: "ProjectEstablishmentRequest", entityId: requestId, linkPath: `/progress/projects/${project.id}#establishment`, mandatory: true, recipients, context: { round, beforeStatus: round === 1 ? null : "DRAFT", afterStatus: "PENDING_APPROVAL", ownerNames: details.ownerNames, taskCount: details.taskCount } });
}
