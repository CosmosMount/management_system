...
const columns = "grid-cols-2 xl:grid-cols-[minmax(12rem,1.1fr)_minmax(0,2.4fr)_minmax(10rem,1.1fr)_5.5rem_6.5rem_6rem_4rem]";
...
export function ProjectList({ projects }: { projects: ProjectListItem[] }) {
  ...
  return <Tooltip.Provider>
    <section aria-label="项目列表" className="min-w-0 rounded-xl border bg-card xl:overflow-x-auto">
      ...
      {projects.map((project) => {
        ...
        return <article key={project.id} data-testid={`project-list-item-${project.id}`} className={cn("grid min-w-0 gap-3 border-b px-4 py-4 text-sm hover:bg-muted/10 min-h-32 items-center", columns)}>
          <TextTooltip text={`${project.name}：${project.description || "暂无项目简介"}`}>
            <Link href={routes.progress.projectDetail(project.id)} className="col-span-2 flex min-w-0 items-center gap-3 rounded-lg focus-visible:outline-2 focus-visible:outline-ring xl:col-span-1">
              ...
              <div className="min-w-0">
                <h2 className="break-words text-lg font-semibold [overflow-wrap:anywhere] xl:truncate">{project.name}</h2>
                <p className="mt-1 truncate text-sm text-muted-foreground">{project.description || "暂无项目简介"}</p>
              </div>
            </Link>
          </TextTooltip>
          <div className="col-span-2 min-w-0 xl:col-span-1"><ProjectTaskOverview tasks={overview.tasks} projectName={project.name} nowMs={nowMs} /></div>
          <div className="col-span-2 min-w-0 xl:col-span-1"><ProjectTaskSummary project={project} overdueCount={overview.overdueCount} dueSoonCount={overview.dueSoonCount} /></div>
