    ...
    <Card key={project.id} className="min-w-0">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t pt-3 text-sm">
          <span className="text-muted-foreground">{project.completedTaskCount}/{project.taskCount} 任务已完成 · {project.participantCount} 位参与人</span>
          <Link href={routes.progress.projectDetail(project.id)} className="inline-flex items-center gap-1 font-medium text-primary hover:underline">打开项目<ArrowRight className="size-4" /></Link>
        </div>
      </CardContent>
    </Card>
  ))}</div>;
