...
const columns = "grid-cols-2 xl:grid-cols-[minmax(19rem,2.6fr)_7rem_6.5rem_minmax(7rem,1fr)_minmax(8rem,1fr)_minmax(8rem,1fr)_8rem]";
...
export function TaskList({ tasks }: { tasks: TaskListItem[] }) {
  ...
  return (
    <section aria-label="任务列表" className="min-w-0 rounded-2xl border border-slate-200 bg-card shadow-sm xl:overflow-x-auto">
      <div aria-hidden="true" className={`hidden gap-4 bg-slate-50/80 px-5 py-3 text-xs font-medium text-slate-500 xl:grid ${columns}`}>
      ...
      {tasks.map((task) => {
        ...
        return (
          <article key={task.id} data-testid={`task-list-item-${task.id}`} className={`grid min-w-0 gap-4 items-center border-t border-slate-100 px-5 py-4 text-sm first:border-t-0 ${columns}`}>
            <div className="col-span-2 flex min-w-0 items-start gap-3 xl:col-span-1">
              ...
              <div className="min-w-0">
                <Link href={routes.progress.taskDetail(task.id)} className="block break-words text-[15px] font-semibold text-foreground hover:text-primary hover:underline [overflow-wrap:anywhere] xl:truncate">{task.title}</Link>
            ...
            <div className="min-w-0 break-words text-muted-foreground [overflow-wrap:anywhere] xl:truncate" title={node}><span className="mb-1 block text-xs xl:hidden">当前节点</span>{node}</div>
            <div className="min-w-0 text-muted-foreground"><span className="mb-1 block text-xs xl:hidden">计划时间</span><div>{plannedAt ? formatDateTime(plannedAt) : "未设置"}</div><NodeDeadline target={task.currentNodeDeadline} className="mt-1" /></div>
